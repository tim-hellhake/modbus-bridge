...
export class ModbusStore {
  ...
  private async getOrCreateDeviceInfo(id: string) {
    ...
    for (const deviceInfo of this.addressMappings) {
      const { deviceId, unitID } = deviceInfo;
  ...
  async put(deviceId: string, propertyName: string, value: boolean | number): Promise<void> {
    ...
    const { unitID } = deviceMapping;

    const { address } = propertyMapping;
    ...
    if (this.debug) {
      // eslint-disable-next-line max-len
      console.log(
        `The value of ${deviceId} (${unitID}) ${propertyName} (${address}) is now ${value}`
      );
    }
  }
